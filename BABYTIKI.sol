...
Features:
- 13% Redistribution In BNB (Hold BABY TIKI tokens, earn BNB. Minimum 10,000 tokens)
...
- Secured By Design (Liquidity locked FOR 1 YEAR minimum) The contract BABY TIKI will be audited.
...
Visit: https://babytikitoken.finance
*/
...
contract BABYTIKI is ERC20, Ownable {
    ...
    BABYTIKIDividendTracker public dividendTracker;
    ...
    uint256 public immutable fixedSaleStartTimestamp = 1627777800; //July 31, 20:30 GMT-4, 2021
    ...
    uint256 public immutable tradingEnabledTimestamp = 1627777800; //July 31, 20:30 GMT-4, 2021
    ...
    constructor() public ERC20("BABY TIKI", "BABYTIKI") {
    	...
    	dividendTracker = new BABYTIKIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BABY TIKI: The dividend tracker already has that address");

        BABYTIKIDividendTracker newDividendTracker = BABYTIKIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BABY TIKI: The new dividend tracker must be owned by the BABY TIKI token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BABY TIKI: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BABY TIKI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BABY TIKI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BABY TIKI: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BABY TIKI: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BABY TIKI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BABY TIKI: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BABY TIKI: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "BABY TIKI: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "BABY TIKI: The fixed-sale is only available to certain participants at the start");
            }
...
contract BABYTIKIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("BABYTIKI_Dividend_Tracker", "BABYTIKI_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BABYTIKI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BABYTIKI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BABY TIKI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "BABYTIKI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BABYTIKI_Dividend_Tracker: Cannot update claimWait to same value");
